...
class Prinzclaw {
  ...
  private scan: ScanSkill;
  private analyze: AnalyzeSkill;
  private craft: CraftSkill;
  private output: OutputSkill;
  private soulHash: string;
  ...
  async runPipeline(entityHandle: string, entityName: string, quote: string, sourceUrl?: string): Promise<{ success: boolean; stage: string; result?: any; error?: string }> {
    try {
  ...
  async approveStrike(strikeId: string, channel?: string) {
    return this.output.approve(strikeId, channel);
  }

  async rejectStrike(strikeId: string, reason?: string) {
