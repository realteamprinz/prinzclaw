...
class CraftSkill {
    ...
    generateCounterText(analysis) {
        ...
        if (!counterText.endsWith('?'))
            counterText += ' Which one is true?';
        return counterText;
    }
    generateEvidenceChain(analysis) {
        const chain = [{
                type: 'promise',
    ...
    async verifyRuleZero(strike, evidenceChain) {
        const errors = [];
